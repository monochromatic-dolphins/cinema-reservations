...
class _SeanceCrudPageState extends State<SeanceCrudPage> {
  ...
  Widget build(BuildContext context) {
    Provider.of<AppState>(context, listen: false).fetchSeanceCrudPageData();
                      ...
                      child: Column(
                        children: [
                          ...
                          DateTimePicker(
                            ...
                            onSaved: (val) => val != null ? _selectedDate = DateTime.parse(val) : null,
                          ),
                          ...
                          DropdownButton(
                            items: state.movies
                                ...
                                .toList(),
                            onChanged: (Movie? value) => _selectedMovie = value,
                            ...
                            items: state.cinemaHalls
                                .map(
                                  (hall) => DropdownMenuItem<CinemaHall>(
                                    value: hall,
                                    child: Text('Hall ${hall.id}'),
                                  ),
                                )
                                .toList(),
                            onChanged: (CinemaHall? value) => _selectedHall = value,
  ...
  void _validateAndCreate() {
    if (_formKey.currentState?.validate() == false || _selectedHall == null || _selectedMovie == null) {
      return;
    }
    Provider.of<AppState>(context, listen: false).createSeance(_selectedHall!, _selectedMovie!, _selectedDate);
  }
}
